...
- Every block-level construct is a sentence boundary where it sits, including inside a tight list item, which has no paragraph edges of its own. That covers the constructs whose content is removed (heading, code block, table, HTML block), a thematic break, which has no content, and the constructs whose content is scored (blockquote, footnote definition), where the boundary falls at the opening edge. An image is inline and is not a boundary: the prose either side of one is a single sentence.
...
- Channels at first release:
  ...
  5. Agent skill: `skills/gunfog/SKILL.md`, added with `pnpm dlx skills add enjunear/gunfog`, which resolves against the mirror. It ships from the repo tree rather than a release artifact, so cargo-dist does not touch it and it needs no tag. It teaches the rewrite loop, the four exit outcomes, and how to get the binary.
