...
**Prose**:
The text that remains after the constructs gfog never scores (headings, fenced code blocks, tables, image alt text, raw HTML) are removed. The fog formula only ever sees prose.
...
**Placeholder word**:
A single simple word standing in for an inline code span or bare URL, so the sentence keeps its grammatical slot and length without the token ever counting as complex.
...
**Hotspot**:
A sentence the report singles out as raising the document's fog score above the target.
